...
func solve(s string, width int, height int) int {
	s = strings.TrimSpace(s)
	lines := strings.Split(s, "\n")

	screen := utils.HashGrid{}
	...
	for _, line := range lines {
		if strings.HasPrefix(line, "rect") {
			...
			for x := range xw {
				for y := range yh {
					screen[utils.Point{X: x, Y: y}] = true
				}
			}
			...
			for x := range width {
				store[utils.Point{X: x, Y: y}] = screen[utils.Point{X: x, Y: y}]
			}
			for x := range width {
				screen[utils.Point{X: x, Y: y}] = store[utils.Point{X: ((x-by)%width + width) % width, Y: y}]
			}
			...
			for y := range height {
				screen[utils.Point{X: x, Y: y}] = store[utils.Point{X: x, Y: ((y-by)%height + height) % height}]
			}
		} else {
